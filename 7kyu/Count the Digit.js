...
// Count the numbers of digits d used in the writing of all the k**2. Call nb_dig (or nbDig or ...) the function taking n
...
function nbDig(n, d) {
    ...
    for (let i = 0; i <= n; i++) {
        ...
        k = i;
        sqrtK = k**2
        if (sqrtK.toString().split('').map(el => {
            if (el === d.toString()) count++ }))
            sum += count
    }
    return sum
}
