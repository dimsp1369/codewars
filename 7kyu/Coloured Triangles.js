...
//     Adapted from the 2017 British Informatics Olympiad
    ...
    function triangle(row) {
        ...
        while (nextRow.length !== 1) {
            let colors = []
            for (let i = 0; i < nextRow.length - 1; i++) {
                if (nextRow[i] == nextRow[i + 1]) colors.push(nextRow[i])
                else if (nextRow[i]  == 'B' && nextRow[i + 1] == 'G' || nextRow[i]  == 'G' && nextRow[i + 1] == 'B') colors.push('R')
                else if (nextRow[i]  == 'R' && nextRow[i + 1] == 'G' || nextRow[i]  == 'G' && nextRow[i + 1] == 'R') colors.push('B')
                else if (nextRow[i]  == 'B' && nextRow[i + 1] == 'R' || nextRow[i]  == 'R' && nextRow[i + 1] == 'B') colors.push('G')
            }
            nextRow = [...colors]
        }
        return nextRow.join('')
