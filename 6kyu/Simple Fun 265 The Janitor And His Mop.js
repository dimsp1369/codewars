...
// doesn't approve of it at all. Every night he vandalizes the writings by erasing all occurrences of some letter.
...
function theJanitor(word) {
    ...
    let mop = []
    alphabet.map(el => word.includes(el) ? mop.push((Math.abs(word.indexOf(el) - word.lastIndexOf(el)) + 1)) : mop.push(0))
    return mop
}