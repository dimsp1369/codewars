...
// Note:
//
//     Selling, buying and saving are normally done at end of month. Calculations are processed at the end of each considered month
// but if, by chance from the start, the value of the old car is bigger than the value of the new one or equal there is no saving to
// be made, no need to wait so he can at the beginning of the month buy the new car:
//
...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
    let months = 0;
    let saving = savingperMonth
    let dif = startPriceOld - startPriceNew
    if (dif >= 0) {
        return [months, dif]
    } else {
        while (dif < 0) {
            ...
            dif = startPriceOld - startPriceNew + savingperMonth
            savingperMonth += saving
        }
        return [months, Math.round(dif)]
    }
}